      ...
      {selectedCourse && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-2xl bg-surface-container-low border border-white/10 rounded-sm shadow-2xl overflow-hidden"
          >
            {/* Decorative background element */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 blur-3xl rounded-sm -mr-32 -mt-32 pointer-events-none"></div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedCourse.techStack.split(',').map(tech => (
                        <span key={tech} className="px-3 py-1 bg-surface-container rounded-sm text-xs font-medium">
                          {tech.trim()}
                        </span>
                      ))}
                    </div>
